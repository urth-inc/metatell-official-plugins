...
import { g as getPrefetchId } from './runtime-utils.esm.js';
...
var fileExistsWithCaseSync = function(filepath) {
    var dir = require$$1.dirname(filepath);
    if (filepath === '/' || filepath === '.') {
        return true;
    }
    var filenames = fs.readdirSync(dir);
    if (filenames.indexOf(require$$1.basename(filepath)) === -1) {
        return false;
    }
...
var _require = require(normalizeWebpackPath('webpack')), RuntimeGlobals = _require.RuntimeGlobals, Template = _require.Template;
    ...
    _create_class(PrefetchPlugin, [
        {
            ...
            value: function apply(compiler) {
                ...
                var runtimePath = require$$1.resolve(__dirname, './plugin.esm.js');
                var sharedPath = require$$1.resolve(__dirname, './shared.esm.js');
                ...
                var asyncEntryPath = require$$1.resolve(compiler.options.context, "node_modules/".concat(TEMP_DIR, "/").concat(encodedName, "/bootstrap.js"));
                ...
                exposeAlias.forEach(function(alias) {
                    ...
                    try {
                        for(var _iterator = targetPaths[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true){
                            ...
                            var absolutePath = require$$1.resolve(compiler.options.context, pathItem);
                            if (fileExistsWithCaseSync(absolutePath)) {
                                prefetchs.push(pathItem);
                                var absoluteAlias = alias.replace('.', '');
                                _this._reWriteExports += "export * as ".concat(getPrefetchId("".concat(name).concat(absoluteAlias)), " from '").concat(absolutePath, "';\n");
                                break;
                            }
                        }
                    } catch (err) {
                ...
                var tempDirRealPath = require$$1.resolve(compiler.options.context, 'node_modules', TEMP_DIR);
    ...
    ], [
        {
            ...
            value: function addRuntime(compiler, options) {
                var encodedName = encodeName(options.name);
                if (!compiler.options.context) {
                    throw new Error('compiler.options.context is not defined');
                }
                var prefetchEntry = require$$1.resolve(compiler.options.context, "node_modules/.mf/".concat(encodedName, "/bootstrap.js"));
                ...
                return Template.asString([
                    fs.existsSync(prefetchEntry) ? Template.indent([
                        ...
                        Template.indent([
                            ...
                            "globalThis.__FEDERATION__['".concat(MFPrefetchCommon.globalKey, "'] = globalThis.__FEDERATION__['").concat(MFPrefetchCommon.globalKey, "'] || {"),
                            "entryLoading: {},",
                            "instance: new Map(),",
                            "__PREFETCH_EXPORTS__: {},",
                            "};",
                            "globalThis.__FEDERATION__['".concat(MFPrefetchCommon.globalKey, "']['").concat(MFPrefetchCommon.exportsKey, "'] = globalThis.__FEDERATION__['").concat(MFPrefetchCommon.globalKey, "']['").concat(MFPrefetchCommon.exportsKey, "'] || {};"),
                            "globalThis.__FEDERATION__['".concat(MFPrefetchCommon.globalKey, "']['").concat(MFPrefetchCommon.exportsKey, "']['").concat(options.name, "'] = function(){ return import('").concat(prefetchEntry, "');}")
                        ]),
                        '}',
